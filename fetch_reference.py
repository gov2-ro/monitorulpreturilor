...
def main(db_path="data/prices.db", limit=None):
    ...
    conn.commit()
    tqdm.write(f"  {len(uats)} UATs saved.")
    ...
    for endpoint, source in [
        ...
        tqdm.write(f"Fetching categories ({source})...")
        ...
        for c in cats:
            upsert_category(conn, c["id"], c["name"], c["parent_id"],
                            c["logo_url"], c["source"])
        conn.commit()
        tqdm.write(f"  {len(cats)} categories saved.")
    ...
    total = 0
    with tqdm(categ_ids, desc="categories", unit="cat") as pbar:
        for cid in pbar:
            pbar.set_postfix(categ=cid, products=total)
